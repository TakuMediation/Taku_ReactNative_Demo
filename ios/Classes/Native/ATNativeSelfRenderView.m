...
@property(nonatomic, strong) ATNativeAttributeMode *parentMode;
@property(nonatomic, strong) ATNativeAttributeMode *appIconMode;
@property(nonatomic, strong) ATNativeAttributeMode *mainImageMode;
@property(nonatomic, strong) ATNativeAttributeMode *mainTitleMode;
@property(nonatomic, strong) ATNativeAttributeMode *descMode;
//@property(nonatomic, strong) ATNativeAttributeMode *adLogoMode;
@property(nonatomic, strong) ATNativeAttributeMode *ctaMode;
@property(nonatomic, strong) ATNativeAttributeMode *dislikeMode;

@property (nonatomic, strong) NSMutableArray<ATNativeAttributeMode *> *attributeModes;
...
- (instancetype) initWithOffer:(ATNativeAdOffer *)offer{
    ...
    if (self = [super init]) {
      ...
      UITapGestureRecognizer *test = [[UITapGestureRecognizer alloc]
          initWithTarget:self action:@selector(testTap)];
      [self addGestureRecognizer:test];
    }
    return self;
}
...
- (void)setMode{
    ...
    self.parentMode = [ATDisposeDataTool disposeNativeData:self.extraDic keyStr:Parent];
    
    self.mainImageMode = [ATDisposeDataTool disposeNativeData:self.extraDic keyStr:MainImage];
    
    self.appIconMode = [ATDisposeDataTool disposeNativeData:self.extraDic keyStr:AppIcon];
    
    self.mainTitleMode = [ATDisposeDataTool disposeNativeData:self.extraDic keyStr:MainTitle];
    
    self.descMode = [ATDisposeDataTool disposeNativeData:self.extraDic keyStr:Desc];
    ATNativeAttributeMode *adLogoMode = [ATDisposeDataTool disposeNativeData:self.extraDic keyStr:AdLogo];
    self.hiddenAdLogo = [adLogoMode unsetFrame];
    
    self.ctaMode = [ATDisposeDataTool disposeNativeData:self.extraDic keyStr:Cta];
    
    self.dislikeMode = [ATDisposeDataTool disposeNativeData:self.extraDic keyStr:Dislike];
}

- (void)setViewValue{
    
    self.backgroundColor =  [ATCommonTool colorWithHexString:self.parentMode.backgroundColorStr];
    if (self.parentMode.cornerRadius != 0) {
        self.layer.cornerRadius = self.parentMode.cornerRadius;
        self.layer.masksToBounds = YES;
    }
    
    self.iconImageView.backgroundColor = [ATCommonTool colorWithHexString:self.appIconMode.backgroundColorStr];
    if (self.appIconMode.cornerRadius != 0) {
        self.iconImageView.layer.cornerRadius = self.appIconMode.cornerRadius;
        self.iconImageView.layer.masksToBounds = YES;
    }
    
    self.mainImageView.backgroundColor = [ATCommonTool colorWithHexString:self.mainImageMode.backgroundColorStr];
    ...
    self.dislikeButton.backgroundColor = [ATCommonTool colorWithHexString:self.dislikeMode.backgroundColorStr];
    ...
    self.titleLabel.textColor = [ATCommonTool colorWithHexString:self.mainTitleMode.textColorStr];
    self.titleLabel.backgroundColor =  [ATCommonTool colorWithHexString:self.mainTitleMode.backgroundColorStr];
    ...
    self.textLabel.textColor = [ATCommonTool colorWithHexString:self.descMode.textColorStr];
    self.textLabel.backgroundColor =  [ATCommonTool colorWithHexString:self.descMode.backgroundColorStr];
    ...
    self.ctaLabel.textColor = [ATCommonTool colorWithHexString:self.ctaMode.textColorStr];
    self.ctaLabel.backgroundColor =  [ATCommonTool colorWithHexString:self.ctaMode.backgroundColorStr];
...
//    self.logoImageView.backgroundColor = [ATCommonTool colorWithHexString:self.adLogoMode.backgroundColorStr];
...
- (CGRect)getRect:(ATNativeAttributeMode *)mode{
...
//    }
    ...
    NSString *ctaText = [ATCommonTool checkStrParamsEmptyAndReturn:self.nativeAdOffer.nativeAd.ctaText];
    self.ctaLabel.text = ctaText.length > 0 ? ctaText : @"立即下载";
