...
+ (void)applyAtAdRequestAndRemove:(NSMutableDictionary *)extraDic {
    ...
    if (adxBidFloorInfo.count > 0) {
        ...
        ATLog(@"atAdRequest adxBidFloorInfo bidFloor=%@ currency=%@ extraMap=%@", bidFloor, currency, extraMap);
    ...
    if (preLoadInfo.count > 0) {
        ...
        ATLog(@"atAdRequest preLoadInfo requestId=%@ psId=%@ placementId=%@ cpEcpmSwitch=%@ cpEcpmTimeout=%@",
