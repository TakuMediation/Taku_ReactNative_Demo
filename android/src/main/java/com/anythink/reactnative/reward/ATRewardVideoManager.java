...
/**
 * 激励视频路由。
 * 一 placementId 一 {@link ATRewardVideoHelper}（池化）。
 * load/show 失败经事件回传；未 init / 无 Activity 才 reject。
 */
public class ATRewardVideoManager {

    private static volatile ATRewardVideoManager sInstance;
    private final ConcurrentHashMap<String, ATRewardVideoHelper> pidHelperMap = new ConcurrentHashMap<>();
    ...
    public static ATRewardVideoManager getInstance() {
        if (sInstance == null) {
            synchronized (ATRewardVideoManager.class) {
                if (sInstance == null) {
                    sInstance = new ATRewardVideoManager();
                }
            }
        }
        return sInstance;
    ...
    private ATRewardVideoHelper getHelper(String placementId) {
        ATRewardVideoHelper helper = pidHelperMap.get(placementId);
        if (helper == null) {
            helper = new ATRewardVideoHelper(appContext());
    ...
    public void load(String placementId, ReadableMap settings) {
        if (TextUtils.isEmpty(placementId)) {
            MsgTools.printMsg("loadRewardedVideo: empty placementId");
            return;
        }
        Map<String, Object> map = RNMapUtil.readableMapToHashMap(settings);
        getHelper(placementId).loadRewardedVideo(placementId, map, currentActivity());
    }
    ...
    public void entryScenario(String placementId, String scenario, ReadableMap tkExtra) {
        ...
        MsgTools.printMsg("entryRewardedVideoScenario: " + placementId + ", " + scenario);
        Map<String, Object> tk = tkExtra != null ? RNMapUtil.readableMapToHashMap(tkExtra) : null;
        if (tk != null && !tk.isEmpty()) {
            ATRewardVideoAd.entryAdScenario(placementId, scenario, tk);
        } else {
    ...
    public void addAutoLoad(String placementId, ReadableMap settings) {
        ATAutoLoadRewardVideoHelper.getInstance()
    ...
    public void removeAutoLoad(String placementId) {
        ATAutoLoadRewardVideoHelper.getInstance().removePlacement(placementId);
    }

    public boolean autoLoadReady(String placementId) {
        return ATAutoLoadRewardVideoHelper.getInstance().isAdReady(placementId);
    }
    ...
    public void showAutoLoad(String placementId, String scenario, Promise promise) {
        ...
        if (configMap != null) {
            ATAutoLoadRewardVideoHelper.getInstance().showWithConfigMap(
                    placementId, configMap, activity);
        } else {
            ATAutoLoadRewardVideoHelper.getInstance().show(placementId, scenario, activity);
        }
        promise.resolve(null);
    }
    ...
    /** scenario 为 showConfig JSON 时解析为 Map（兼容旧桥无 showAutoLoadRewardedVideoWithConfig）。 */
